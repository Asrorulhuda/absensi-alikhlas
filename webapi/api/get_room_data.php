...
if(isset($_GET["id_venue"]) && trim((string) $_GET["id_venue"]) !== ''){
  $id_venue = mysqli_real_escape_string($GLOBALS["___mysqli_ston"], trim((string) $_GET['id_venue']));
  
  $sql = "SELECT * FROM room WHERE r_id='$id_venue'";
  $s_venue = mysqli_query($GLOBALS["___mysqli_ston"], $sql);
  if (!$s_venue || mysqli_num_rows($s_venue) === 0) {
  ...
  $tap_out_today = $s_tap_out ? mysqli_num_rows($s_tap_out) : 0;
